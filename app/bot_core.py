        ...
        if data.get('k', {}).get('x', False):
            ...
            if self.status["in_position"]:
                ...
                if not any(p['symbol'] == self.status['symbol'] for p in open_positions):
                    print(f"--> Pozisyon kapandığı tespit edildi. Emirler temizleniyor ve sonuçlar kaydediliyor.")
                    ...
                    trade_log = {"symbol": self.status["symbol"], "side": self.status.get("position_side"), "entry_price": self.status.get("entry_price"), "exit_price": current_price, "status": "CLOSED", "pnl": closed_pnl, "timestamp": datetime.now(timezone.utc)}
